...
void twi_init(){
	GPIO->CNF[0] = (1<<3)|(1<<2)|(1<<9)|(1<<10); 	// set SCL to input, activate pullup, drive stength S0D1
	GPIO->CNF[30] = (1<<3)|(1<<2)|(1<<9)|(1<<10);// set SDA to input, activate pullup, drive stength S0D1
...
void twi_multi_read(uint8_t slave_address,uint8_t start_register,
	...
	data_buffer[registers_to_read -1] = TWI0->RXD; //read buss one last sime
