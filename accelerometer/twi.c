...
void twi_init(){
    GPIO->PIN_CNF[0] = (1<<3)|(1<<2)|(1<<9)|(1<<10);// SCL ->input, activate pullup, drive stength S0D1
    GPIO->PIN_CNF[30] = (1<<3)|(1<<2)|(1<<9)|(1<<10);// SDA ->input, activate pullup, drive stength S0D1
...
void twi_multi_write(uint8_t slave_address,uint8_t start_register,
    ...
    for(int i = 0; i < (registers_to_write);i++){
        while(!TWI0->TXDSENT); //wait for a ACK , byte received
        TWI0->TXDSENT = 0; //Reset ACK for byte received
